...
describe('TodoDataService', () => {
  ...
  describe('#deleteTodoById(id)', () => {
   ...
   it('should not removing anything if todo with the corresponding id is not found',
   inject([TodoDataService], (service: TodoDataService) => {
    const todo1 = new Todo({title: 'Hello 1', complete: false });
    const todo2 = new Todo({title: 'Hello 2', complete: true });
 ...
 describe('#updateTodoById(id, values)', () => {
  it('should return todo with the corresponding id and updated data',
    inject([TodoDataService], (service: TodoDataService) => {
      const todo = new Todo({title: 'Hello1', complete: false});
      service.addTodo(todo);
      const updateTodo = service.updateTodoById(1, {
        title: 'new title'
      });
  ...
  it('should return null if todo is not found',
    inject([TodoDataService], (service: TodoDataService) => {
      const todo = new Todo({title: 'new title', complete: false});
      service.addTodo(todo);
 ...
 describe('#toggleTodoComplete(todo)', () => {
  ...
  it('should return the updated todo with inverse complete status',
    inject([TodoDataService], (service, TodoDataService) => {
      const todo = new Todo({title: 'new title', complete: false});
      service.addTodo(todo);
      const updateTodo = service.toggleTodoComplete(todo);
